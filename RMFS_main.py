...
# Sequence positions modelled per picking station (|Sq| in the manuscript).
#   "tight"  : |Sq| = number of order groups (Section 3.1 definition, default)
#   "legacy" : |Sq| = |O|, the looser bound used for the published tables
# Both are valid: surplus positions stay empty and Constraints (3.23)-(3.24)
# force their times to zero, so the optimum is unchanged.
SEQ_MODE = os.environ.get("RMFS_SEQ_MODE", "tight").strip().lower()
assert SEQ_MODE in ("tight", "legacy")


def n_sequence_positions(order_groups, n_orders):
    """Number of sequence positions to model at each picking station."""
    return len(order_groups) if SEQ_MODE == "tight" else n_orders


# The neighborhood set defining "the proposed VNS" throughout the paper.
#   k=2 : exchange one order between two groups        (N1 in the manuscript)
#   k=3 : exchange station and sequence of two groups  (N2 in the manuscript)
#   k=4 : exchange one item between two waves          (evaluated in Table 8)
# k=1 is kept for backward compatibility only: it yields the same move as k=3
# by a different route, so a set containing both counts one neighborhood
# twice. Every script reporting a headline result imports this constant.
ACTIVE_NEIGHBORHOODS = (2, 3)
...
def assigning_order_groups(order_groups, num_picking_stations, Demand):
    ...
    n_seq = n_sequence_positions(order_groups, len(Demand))
    Sequences = list(range(n_seq))
    Picking_stations = list(range(num_picking_stations))
    ss = 0
    jj = 0

    yy = np.zeros((len(order_groups), num_picking_stations, n_seq))
    active = np.zeros((len(order_groups), num_picking_stations, n_seq))
...
#--------------------------------
    ...
    Sequences = list(range(n_sequence_positions(order_groups, orders1)))
...
def variable_neighborhood_search(Sum_demand, orders, group_capacity, max_iterations,
                                   ...
                                   initial_order_groups=None, initial_yy=None):
    """
    ...
    # A caller may supply its own starting solution (ablation_study.py uses
    # this to start the VNS from a random grouping instead of the FFD one).
    if initial_order_groups is None:
        best_order_groups = create_order_groups(Sum_demand, orders, group_capacity)
    else:
        best_order_groups = copy.deepcopy(initial_order_groups)
    if initial_yy is None:
        best_yy, active, jj = assigning_order_groups(best_order_groups, num_picking_stations, Demand)
    else:
        best_yy = initial_yy.copy()
    ...
    while iteration < k_max:
        ...
        while k_idx < len(active_neighborhoods):
            ...
            if k == 1:
                ...
                candidate_yy = swap_neighborhood(
                    candidate_yy, candidate_yy.shape[2], num_picking_stations)
            elif k == 4:
