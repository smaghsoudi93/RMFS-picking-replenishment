...
Place this file in the same folder as:
  - RMFS_main.py
  - run_ffd_single.py
  - every instance file (Data*.xlsx) to be tested

Before running, set the INSTANCE_FILES list below to the instance files to
cover (the same ones used for Table 3).

What it does:
  - For each instance it runs run_ffd_single.py as a fully independent
    subprocess (setting RMFS_DATA_FILE to that file). A subprocess is used
    deliberately rather than a direct import, because RMFS_main reads the
    Excel instance only once per process lifetime -- the next instance needs
    a completely fresh Python process.
  - It reads each instance's result file (ffd_comparison_<label>.xlsx).
  - It builds an across-instance summary: the mean of each method on each
    instance, plus a paired t-test comparing Modified FFD against Standard
    FFD at the level of instance means rather than individual replications.

Output:
  - ffd_comparison_<label>.xlsx per instance (written by run_ffd_single.py)
  - ffd_multi_instance_summary.xlsx (final across-instance summary)
"""
...
# ══════════════════════════════════════════════════════
# Set this list to the instance files to cover
# (the same ones used for Table 3: 6, 8, 15, 20, 30, ... orders)
# ══════════════════════════════════════════════════════
INSTANCE_FILES = [
    "Data3.xlsx",
    "Data4.xlsx",
...
N_REPLICATIONS_PER_INSTANCE = 3  # reduced to keep total runtime manageable
